...
For working without the internal gain of the PMT we need to avoid the use of the electron multiplication stage that we saw in section \ref{subsubsec:PMTs}. To achieve this, we design, build and test special PCBs, whose electronic scheme is shown in figure \ref{fig:PMTBaseWithoutGain}, in which we short-circuit all the dindes and read the signal directly from the photocathode.

\begin{figure}[htbp]
 \centering
  \subfloat[Electronic Voltage Divider Circuit.]{
   \label{subfig:BasePMTNoGain}
    \includegraphics[width=0.5\textwidth]{3DesignPrinciples/32Tritium_detector/BasePMTNoGain.jpeg}}
    %\newline
  \subfloat[Electronic scheme of the electronic voltage divider circuit.]{
   \label{subfig:ElectronicSchemeBasePMTNoGain}
    \includegraphics[width=0.5\textwidth]{3DesignPrinciples/32Tritium_detector/ElectronicSchemBasePMTNoGain.png}}
 \caption{Electronic voltage divider circuit ant its electronic scheme for working with PMTs without its internal gain.}
 \label{fig:PMTBaseWithoutGain}
\end{figure}
