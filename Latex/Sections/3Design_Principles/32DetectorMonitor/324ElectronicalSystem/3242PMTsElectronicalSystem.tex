...
\item{} The time coincidence line  contains the time information and gives the gate... It is used to obtain the coincidence gate that indicates when we have to save the amplified signal (when all the output signals of the used PMTs are in time coincidence). This line consists of:
...
\item{} In Figures \ref{subfig:signalInTwoPMTOneDetector} and \ref{subfig:signalInTwoPMTOtherDetector} two PMT signals of the same detector are ALGO event but the other detector gives no signal. This event is discarded.
